...
public class TokenServiceEventPublishingTests : TokenServiceTestBase
{
    ...
    public async Task RefreshToken_PublishesAuditPublishedEvent()
    {
        ...
        // Generate a test token with the same user ID as the test user and the correct issuer/audience
        var testToken = GenerateTestToken(JwtOptions.Key, user.Id, user.Email!);
        Output.WriteLine($"Generated test token: {testToken}");
        
        // Configure UserManager to return the test user when queried by ID
        ...
        // Configure UserManager to succeed on UpdateAsync
